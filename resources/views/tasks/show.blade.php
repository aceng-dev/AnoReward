            ...
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    ...
                    <div class="flex items-center gap-3 mb-2">
                        <span class="px-3 py-1 rounded-full {{ $statusStyles[$task->status] ?? 'bg-gray-100 text-gray-700' }} text-xs font-semibold">
                            {{ $statusLabels[$task->status] ?? ucfirst($task->status) }}
                        </span>
                        <span class="text-xs font-medium text-gray-600 bg-gray-100 px-3 py-1 rounded">{{ $task->project->project_name ?? 'Tanpa Project' }}</span>
                    </div>
                ...
                <div class="text-right">
                    ...
                    <div class="text-2xl font-bold text-gray-900">{{ optional($task->due_date)->translatedFormat('j F Y') ?? 'Tanpa deadline' }}</div>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <div class="font-semibold text-gray-900 capitalize">{{ $statusLabels[$task->status] ?? ucfirst($task->status) }}</div>
                        </div>
                        <div>
                            <div class="text-sm text-gray-600 mb-1">Project</div>
                            <div class="font-semibold text-gray-900">{{ $task->project->project_name ?? 'Tanpa Project' }}</div>
                        </div>
                        <div>
                            <div class="text-sm text-gray-600 mb-1">Deadline</div>
                            <div class="font-semibold text-gray-900">{{ optional($task->due_date)->translatedFormat('j F Y') ?? 'Tanpa deadline' }}</div>
                        </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                    ...
                    <div class="space-y-4">
                        ...
                        @if($task->status !== 'todo')
                            <div class="flex gap-4">
                                <div class="flex flex-col items-center">
                                    <div class="w-10 h-10 rounded-full bg-yellow-100 text-yellow-600 flex items-center justify-center font-bold">2</div>
                                    <div class="w-1 h-12 bg-gray-200 my-2"></div>
                                </div>
                                <div>
            ...
            <div class="space-y-6">
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                    ...
                    <div class="font-semibold text-gray-900 mb-2">{{ $task->project->project_name ?? 'Tanpa Project' }}</div>
                    <div class="text-sm text-gray-600 mb-4">{{ optional($task->project)->description ?? 'Tidak ada deskripsi' }}</div>
                    <div class="text-sm text-gray-700">
                        <strong>Manager:</strong> {{ optional($task->project->manager)->name ?? 'Tanpa manager' }}
                    </div>
                </div>
                    ...
                    <div class="space-y-3">
                        @if($task->status === 'todo')
                            <form action="{{ route('tasks.start', $task) }}" method="POST">
                                @csrf
                                <button type="submit" class="w-full px-4 py-2 bg-gradient-to-r from-red-600 to-red-800 text-white rounded-lg font-semibold hover:shadow-lg transition">?? Mulai Mengerjakan</button>
                            </form>
                        @elseif($task->status === 'inprogress')
                            <form action="{{ route('tasks.complete', $task) }}" method="POST">
                                @csrf
                                <button type="submit" class="w-full px-4 py-2 bg-gradient-to-r from-green-600 to-green-800 text-white rounded-lg font-semibold hover:shadow-lg transition">? Tandai Selesai</button>
                            </form>
                        @else
                            <div class="px-4 py-2 bg-green-100 text-green-700 rounded-lg text-center font-semibold">? Sudah Selesai</div>
                        @endif
                    </div>
                </div>
